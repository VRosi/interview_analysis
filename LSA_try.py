...
def preprocess_data2(doc_set, word, db):
    """
    Input  : docuemnt list
    Purpose: preprocess text (tokenize, removing stopwords, and stemming)
    Output : preprocessed text
...
document_list = []
for i in ["rond","chaud"]:
    document_list.append(load_data(i, [], Q))
document_list = [j for sub in document_list for j in sub]
clean_text=preprocess_data2(document_list,"", True)
